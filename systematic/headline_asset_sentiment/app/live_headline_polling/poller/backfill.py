...
from db import latest_publication_date, upsert_headlines
from fetcher import fetch_historical_headlines
...
def _start_date_for(ticker: str, days_back: int) -> date:
    """
    Return the date to start fetching historical data for *ticker*.
...
def backfill_all_tickers(days_back: int | None = None) -> None:
    ...
    Both the historical and live endpoints write via upsert_headlines() so data
