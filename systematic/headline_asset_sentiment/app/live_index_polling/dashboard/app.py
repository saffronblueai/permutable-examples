...
ALL = "ALL"
ROLLING_WINDOW = 5  # hours for conviction rolling mean
...
def _conviction_color(v: float) -> str:
...
def _rolling_conviction(df: pd.DataFrame, active_tickers: list[str]) -> pd.DataFrame:
    """Aggregate to (ticker, hour), compute conviction ratio and rolling mean."""
    agg = (
...
def _build_conviction_ts(agg: pd.DataFrame, active_tickers: list[str], title: str) -> go.Figure:
    fig = go.Figure()
    for i, ticker in enumerate(active_tickers):
        ...
        fig.add_trace(go.Scatter(
            x=sub["publication_time"], y=sub["conviction_ratio"],
            mode="lines", line=dict(color=color, width=1, dash="dot"),
            name=f"{ticker} raw", legendgroup=ticker, opacity=0.4,
        ))
        fig.add_trace(go.Scatter(
            x=sub["publication_time"], y=sub["conviction_smooth"],
            mode="lines", line=dict(color=color, width=2),
            name=f"{ticker} {ROLLING_WINDOW}h rolling", legendgroup=ticker,
        ))
    ...
    fig.update_layout(
        **_CHART_BASE,
        ...
        yaxis_title="Conviction Ratio  [−1, +1]",
...
def _build_conviction_heatmap(agg: pd.DataFrame, active_tickers: list[str], title: str) -> go.Figure:
    """5-period rolling conviction heatmap — ticker × hour, date ticks at day boundaries."""
    pivot = (
        agg.pivot_table(index="publication_time", columns="ticker", values="conviction_smooth", aggfunc="mean")
    ...
    fig = go.Figure(go.Heatmap(
        ...
        y=active_tickers,
        colorscale=[
            [0.0, "#c0392b"], [0.5, "#f39c12"], [1.0, "#2ecc71"],
        ],
        zmid=0,
        zmin=-1,
        zmax=1,
        colorbar=dict(title=dict(text="Conviction", font=dict(color="#ccc")),
                      tickfont=dict(color="#ccc"), x=1.0, xanchor="left", thickness=14, len=0.75),
        hovertemplate="<b>%{y}</b> — %{x}<br>Conviction: %{z:.3f}<extra></extra>",
    ))
    fig.update_layout(
        **_CHART_BASE,
        title=title,
...
def _build_topic(df: pd.DataFrame, title: str, top_n: int = 15) -> go.Figure:
    """Mean conviction ratio by topic (top N by total headline volume)."""
    agg = (
    ...
    fig = go.Figure(go.Bar(
        x=topic_df["conviction"],
        y=topic_df["topic_name"],
        orientation="h",
        marker_color=["#e74c3c" if v < 0 else "#2ecc71" for v in topic_df["conviction"]],
    ))
    fig.add_vline(x=0, line_color="#555", line_dash="dash", line_width=1)
    fig.update_layout(
        **_CHART_BASE,
        title=title,
        xaxis_title="Mean Conviction Ratio",
...
_ticker_options = [{"label": "All tickers", "value": ALL}] + [
    {"label": t, "value": t} for t in settings.tickers_list
]
...
app.layout = html.Div(
    [
        html.Div(
            [
                html.Div([
                    html.H1("Headline Index Monitor",
                            style={"margin": "0", "color": "#fff", "fontSize": "20px"}),
                    html.P("Permutable AI  ·  Live Polling Application",
...
def update_content(selected: str, n: int):
    ...
    agg = _rolling_conviction(df, active_tickers)
    ...
    mean_conviction = latest_per_ticker["conviction_smooth"].mean()
    total_headlines = int(agg["headline_count"].sum())
    high_count = int((latest_per_ticker["conviction_smooth"] >= settings.upper_threshold).sum())
    low_count  = int((latest_per_ticker["conviction_smooth"] <= settings.lower_threshold).sum())

    conv_color = _conviction_color(mean_conviction)

    stat_row = html.Div(
        [
            _stat_card("Index records (7d)", f"{len(df):,}"),
            _stat_card("Mean conviction", f"{mean_conviction:+.3f}", color=conv_color),
    ...
    fig_conv   = _build_conviction_ts(
        agg, active_tickers,
        f"{scope_label}  —  {ROLLING_WINDOW}h Rolling Conviction Ratio  (Last 7 d)",
    )
    fig_count  = _build_headline_count(
        agg, active_tickers,
        f"{scope_label}  —  Hourly Headline Count  (Last 7 d)",
    )
    fig_heatmap = _build_conviction_heatmap(
        agg, active_tickers,
        f"{scope_label}  —  {ROLLING_WINDOW}h Rolling Conviction Heatmap",
    )
    fig_indicator = _build_indicator_heatmap(
        agg, active_tickers,
        f"{scope_label}  —  Conviction Indicator  "
        f"(HIGH ≥ {settings.upper_threshold} | LOW ≤ {settings.lower_threshold})",
    )
    fig_topic = _build_topic(
        df, f"{scope_label}  —  Mean Conviction by Topic  (top 15 by volume)",
    )
    ...
    return [
        stat_row,
        dcc.Graph(figure=fig_conv,      style={"marginBottom": "16px"}),
